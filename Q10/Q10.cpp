...
void cloneList(Node* sourceList, Node*& destList)
{
    for (Node* i = sourceList; i != nullptr; i = i->next)
    {
        if (dynamic_cast<Teacher*>(i->data))
        {
            People* p = new People(i->data);
            Node::addElement(destList, p);
        }
        else if (dynamic_cast<Student*>(i->data))
        {
            People* st = new Student(dynamic_cast<Student*>(i->data));
            Node::addElement(destList, st);
        }
        else
        {
            People* stf = new Staff(dynamic_cast<Staff*>(i->data));
            Node::addElement(destList, stf);
        }
    }
}
...
{
{
        Node* head = nullptr;
        Node* clone = nullptr;
        input(head);//them phan tu vao list
        sortList(head);//sap xep list
        printList(head);//in ra list
        printStudent(head);//in ra sv co diem
        cloneList(head, clone);//tao ban sao list
        printList(clone);//in ra list
        delList(head);//xoa list
        delList(clone);//xoa list
}
_CrtDumpMemoryLeaks();
}